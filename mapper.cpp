...
//////////////////////////////////////////////////////////////////////////////////////
void Mapper::setContig(const platanus::Contig &contig)
{
...
void Mapper::mapAnchorBubbleMT(platanus::Contig &anchorContig, vector<platanus::Region> &anchorMap, const long numThread)
{
    ...
    cerr << "mapping anchor-bubbles..." << endl;

    omp_set_num_threads(numThread);
	anchorMap.resize(anchorContig.numSeq);
    ...
    for (long i = 0; i < numThread; ++i) {
		...
		for (long j = 2*i; j < anchorContig.numSeq; j += 2*numThread) {
            ++totalPair;
			platanus::SEQ &bubble1 = anchorContig.seq[j];
			platanus::SEQ &bubble2 = anchorContig.seq[j + 1];

            if (bubble1.length < seedLength || bubble2.length < seedLength)
				continue;

			double identity;
            platanus::Position position1 = this->mapReadUngapAlignment(bubble1, MIN_IDENTITY, positionBuffer, &identity);
            if (position1.id == 0)
				continue;
            ...
            if (abs(position1.id) == abs(position2.id)) {
                ++numMappedSameContig;
				continue;
			}

			anchorMap[j].id = position1.id;
			anchorMap[j + 1].value = j;
			if (position1.id > 0) {
				anchorMap[j].start = position1.offset;
				anchorMap[j].end = position1.offset + bubble1.length;
			}
			else {
				anchorMap[j].start = position1.offset - bubble1.length + 1;
				anchorMap[j].end = position1.offset + 1;
			}
			...
			if (position2.id > 0) {
				...
				anchorMap[j + 1].start = position2.offset - bubble2.length + 1;
				anchorMap[j + 1].end = position2.offset + 1;
			}
...
void Mapper::mapLongReadAndSaveReadLink(vector<SeqLib> &library, const long minContigLength, const long numThread)
{
    ...
    long numMapped = 0;
    long sum = 0;
	const int match = this->seedLength;
    ...
    for (i = 0; i < numThread; ++i) {
        ...
        while (read.readTemporaryFile(library[i].pairFP)) {
			...
			for (unsigned j = 0; j < positionBuffer.size(); ++j) {
				...
				fwrite(&match, sizeof(int), 1, library[i].mappedReadFP);
			}
