...
template <typename KMER>
void BruijnGraph<KMER>::saveBubbleAsContig(FILE *contigFP, const double coverageRatio)
{
    std::vector<Junction*> junction(4);
    std::vector<Straight*> straight(4);
    Junction *rootJunction;
    ...
    for (auto it = junctionTable.begin(), end = junctionTable.end(); it != end; ++it) {
        rootJunction = &(it->second);
        if (rootJunction->coverage == UINT16_MAX || platanus::FlagCount(rootJunction->out & 0x0f) != 2)
			continue;
        ...
        for (bases[0] = 0; bases[0] < 3; ++bases[0]) {
            if (straight[bases[0]] != NULL &&
				junction[bases[0]] != NULL &&
				straight[bases[0]]->coverage != UINT16_MAX &&
				junction[bases[0]]->coverage != UINT16_MAX) {
				break;
			}
		}
		if (bases[0] == 3 || platanus::FlagCount(junction[bases[0]]->out >> 4) != 2)
			continue;

		for (bases[1] = bases[0] + 1; bases[1] < 4; ++bases[1]) {
            if (straight[bases[1]] != NULL &&
				junction[bases[1]] == junction[bases[0]] &&
				straight[bases[1]]->coverage != UINT16_MAX) {
				break;
			}
		}
		if (bases[1] == 4)
		...
		if (straight[bases[0]]->length < straight[bases[1]]->length ||
			(straight[bases[0]]->length == straight[bases[1]]->length &&
			straight[bases[0]]->coverage < straight[bases[1]]->coverage)) {

			std::swap(straight[bases[0]], straight[bases[1]]);
		}

		for (unsigned i = 0; i < 2; ++i) {
			unsigned long long length = straight[bases[i]]->length + this->kmerLength - 1;
			unsigned short coverage = static_cast<unsigned short>(straight[bases[i]]->coverage * coverageRatio + 0.5);
			binstr_t seq(length);
			seq.convertFromVector(straight[bases[i]]->seq);
