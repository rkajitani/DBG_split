...
void convertContigToReadsMT(vector<SeqLib> &lib, platanus::Contig &contig, const int numThread)
{
    long i;
    platanus::SEQ forwardSeq, reverseSeq;
	...
	for (long i = 0; i < contig.numSeq; ++i) {
        contig.seq[i].writeTemporaryFile(lib[threadIndex].pairFP);
        lib[0].addTotalLength(contig.seq[i].length + reverseSeq.length);
		if (i % 2 == 0)
			lib[0].addNumPair(1);
