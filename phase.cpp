...
void Phase::usage(void) const
{
    ...
    std::cerr << "\nUsage: platanus_allee phase [Options]\n"
              ...
              << "    -f INT                             : start of iteration-round to resume (default " << optionSingleArgs.at("-f") << ")\n"
...
void Phase::exec(void)
{
    ...
    if (!(optionMultiArgs["-p"].empty()) && !(optionPairFile.empty())) {
		++numIterate;
	}
...
{
	vector<string> boolArgOptionList = {"-minimap2_sensitive", "-aggressive", "-reduce_redundancy", "-emem"};
	vector<string> singleArgOptionList = {"-t", "-tmp", "-l", "-k", "-mapper"};
	vector<string> multiArgOptionList = {"-x", "-X", "-s"};
	if (times > 1 || optionPairFile.empty()) {
...
void Phase::execGapClose(const unsigned long times, const unsigned long numIterate)
{
	...
	if (optionPairFile.empty()) {
		oss << " ln -fs " <<  optionSingleArgs["-o"] << "_primaryBubble.fa " << outputPrefix << "_gapClosed_primaryBubble.fa\n"
			<< " ln -fs " <<  optionSingleArgs["-o"] << "_secondaryBubble.fa " << outputPrefix << "_gapClosed_secondaryBubble.fa\n"
			<< " ln -fs " <<  optionSingleArgs["-o"] << "_primaryFork.fa " << outputPrefix << "_gapClosed_primaryFork.fa\n"
			<< " ln -fs " <<  optionSingleArgs["-o"] << "_secondaryFork.fa " << outputPrefix << "_gapClosed_secondaryFork.fa\n"
			<< " ln -fs " <<  optionSingleArgs["-o"] << "_nestedBubble.fa " << outputPrefix << "_gapClosed_nestedBubble.fa\n"
			<< " ln -fs " <<  optionSingleArgs["-o"] << "_nonBubbleOther.fa " << outputPrefix << "_gapClosed_nonBubbleOther.fa\n";
		if (system(oss.str().c_str()) != 0) {
